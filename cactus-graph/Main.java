...
public class Main {
    ...
    void run() {
        ...
        int N = cin.nextInt(); // vértices
        int M = cin.nextInt(); // número de caminhos
        g = new ArrayList<List<Edge>>(); // grafo
        visited = new ArrayList<Boolean>(); // vértices visitados

        for (int i = 0; i < N; i++) {
            g.add(new ArrayList<Edge>()); // para cada um dos vértices crio uma lista de arestas
            visited.add(false); // marco que todos os vértices ainda não foram visitados
        }
        ...
        try {
            ...
            System.out.println("Grafo não é cacto");
        }
    }
